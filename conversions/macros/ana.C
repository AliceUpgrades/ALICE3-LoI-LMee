...
{
	TChain mcTree("o2sim");
	mcTree.AddFile("../input/o2sim_Kine.root");
	...
	TH1D hLS1prim {"hLS1","Like Sign spectrum of -- pairs from primary particles;m_{ee} (GeV/c^{2})", 100,0,1};
	TH1D hLS2prim {"hLS2","Like Sign spectrum of ++ pairs from primary particles;m_{ee} (GeV/c^{2})", 100,0,1};
	...
	for(int iEvent = 0; iEvent < nEvents; ++iEvent) {
		...
		for (const auto track : *mcTracks) {
			...
			if (sqrt(track.GetStartVertexCoordinatesX()*track.GetStartVertexCoordinatesX() + track.GetStartVertexCoordinatesY()*track.GetStartVertexCoordinatesY())> 5.55) continue;
			if (track.isPrimary()) {
