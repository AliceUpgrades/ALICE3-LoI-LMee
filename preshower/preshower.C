...
void
preshower(const char *inputFile = "delphes.root",
	  const char *outputFile = "preshower.root",
	  ...
	  )
{
  ...
  auto hElectronPt = new TH1F("hElectronPt", ";#it{p_{T}} (GeV/#it{c});", nbins, xbins);
  auto hPionPt = new TH1F("hPionPt", ";#it{p_{T}} (GeV/#it{c});", nbins, xbins);
  ...
  for (Int_t ientry = 0; ientry < numberOfEntries; ++ientry) {
    ...
    for (Int_t itrack = 0; itrack < tracks->GetEntries(); ++itrack) {
      ...
      // smear track (switched off for the moment for this simple test)
	...
	track->TOuter = (hMismatchTemplateIn->GetRandom() + L / 299.79246) * 1.e-9; 
      ...
      // check if it is identified as an electron by the Preshower
	...
	// (in the proper analysis macro all other candidates
	// would be cut away here)
	if(!pidmap[pdg]){
	  hElectronPt->Fill(track->PT);
	}
	else{
	  hPionPt->Fill(track->PT);
	}
      }
