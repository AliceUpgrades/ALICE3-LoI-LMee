...
double rich_PionRejection_pt_cut = 0.8; // [GeV/c]
...
TString smearingfile("/mnt/Work/ALICE3/FilesForTest/resolution_test_5kG.root");
...
TLorentzVector  ApplySmearing(const TLorentzVector& vec, Double_t ch)
{
  //
  ...
  )
{
  ...
  auto hBetaP_hasTOF = new TH2F("hBetaP_hasTOF", ";#it{p} GeV/c;#beta", 1000, 0., 10., 1000, 0.1, 1.1);
  auto hBetaP_hasRICH = new TH2F("hBetaP_hasRICH", ";#it{p} GeV/c;#beta", 1000, 0., 10., 1000, 0.1, 1.1);
  TH2 *hNsigmaP_TOF[5];
  ...
  auto hCherenkovAngleP_hasRICH= new TH2F("hCherenkovAngleP_hasRICH", ";#it{p} GeV/c; cherenkov angle", 1000, 0., 10., 500, 0., 0.4);
  ...
  for (Int_t ientry = 0; ientry < numberOfEntries; ++ientry) {
    ...
    for (Int_t itrack = 0; itrack < tracks->GetEntries(); ++itrack) {
      ...
      // fill beta-p after hasTOF
      hBetaP_hasTOF->Fill(p, beta);

      // // check if has RICH
      // if (!richdetector.hasRICH(*track)) continue;


      // fill beta-p, cherenkov_angle-p after hasRICH
      hBetaP_hasRICH->Fill(p, beta);
      hCherenkovAngleP_hasRICH->Fill(p,measAngle);

      // kinatic cuts on tracks
      if (!kineCuts(track)) continue;


      // if (p > tof_EleAccep_pt_cut){
      //   if(abs(PIDnsigmaRICH[0]) > nSigmaRICHEle) continue;
      //   if((p > rich_PionRejection_pt_cut) && (abs(PIDnsigmaRICH[2]) < nSigmaRICHPi)) continue;
      // }
      // else if (p < tof_EleAccep_pt_cut){
      //   if(abs(PIDnsigmaTOF[0]) > nSigmaTOFEle) continue;
      //   if(abs(PIDnsigmaTOF[2]) < nSigmaTOFPi) continue;
      // }
      ...
      if(richdetector.hasRICH(*track)) {
        ...
        if(fabs( (PIDnsigmaRICH[2]) < nSigmaRICHPi) && (p > 1.) ) RICHpid = false; // is within 3 sigma of the pion band (RICH)
      }

      if (!(RICHpid || TOFpid)) continue;


      // fill nsigma after PID cuts
      ...
      // fill histograms
      hAllTracks_Rec_Pt_Eta_Phi->Fill(track->PT,track->Eta,phiRec);
      if     (track->Charge < 0)  hNegTrack_Rec_Pt_Eta_Phi->Fill(track->PT,track->Eta,phiRec);
      else if(track->Charge > 0)  hPosTrack_Rec_Pt_Eta_Phi->Fill(track->PT,track->Eta,phiRec);
      ...
      // fill track vector
      if (track->Charge < 0) vecNegTracks.push_back(track);
      else if (track->Charge > 0) vecPosTracks.push_back(track);
      ...
      if(abs(particle->PID) != 11 ) continue;
                                      ...
                                      hTrack_ElePos_Rec_Pt_Eta_Phi->Fill(track->PT,track->Eta,phiRec);
                                      // !!!!!!!!!!!!!!! NOTICE CHANGED TACK to PARTICLE !!!!!!!!!!!!!!!
      if (particle->PID == 11)        hTrack_Ele_Rec_Pt_Eta_Phi->Fill(particle->PT,particle->Eta,TVector2::Phi_0_2pi(particle->Phi));
      else if (particle->PID == -11)  hTrack_Pos_Rec_Pt_Eta_Phi->Fill(particle->PT,particle->Eta,TVector2::Phi_0_2pi(particle->Phi));
                                      // !!!!!!!!!!!!!!! NOTICE CHANGED TACK to PARTICLE !!!!!!!!!!!!!!!
    ...
    for (Int_t iparticle = 0; iparticle < particles->GetEntries(); ++iparticle) {
      ...
      LV.SetPtEtaPhiM(particle->PT,particle->Eta,particle->Phi,eMass);
      Double_t charge = -particle->PID;
      LV_smeared = ApplySmearing(LV,charge);
      ...
      if (particle->PID == 11 ) vecElectronGen.push_back(particle);
      else if (particle->PID == -11 ) vecPositronGen.push_back(particle);

                                      hTrack_ElePos_Gen_Pt_Eta_Phi->Fill(particle->PT,particle->Eta,phiGen);
      if (particle->PID == 11)        hTrack_Ele_Gen_Pt_Eta_Phi->Fill(particle->PT,particle->Eta,phiGen);
      else if (particle->PID == -11)  hTrack_Pos_Gen_Pt_Eta_Phi->Fill(particle->PT,particle->Eta,phiGen);
  ...
  hCherenkovAngleP_hasRICH->Write();
