...
runDelphes() {
  ...
  root -q -l "cocktailInput.cxx(\"delphes.$1.root\", \"cocktailInput.$1.root\")" &> bkg.$1.log
}
...
# code
cp ./macros/cocktailInput.cxx cocktailInput.cxx
...
# LUTs
cp ../LUTs/lutCovm.${RADIUS}cm.${BFIELD}kG/lutCovm.el.dat lutCovm.el.dat
cp ../LUTs/lutCovm.${RADIUS}cm.${BFIELD}kG/lutCovm.mu.dat lutCovm.mu.dat
cp ../LUTs/lutCovm.${RADIUS}cm.${BFIELD}kG/lutCovm.pi.dat lutCovm.pi.dat
cp ../LUTs/lutCovm.${RADIUS}cm.${BFIELD}kG/lutCovm.ka.dat lutCovm.ka.dat
cp ../LUTs/lutCovm.${RADIUS}cm.${BFIELD}kG/lutCovm.pr.dat lutCovm.pr.dat
...
sed -i -e "s/double Bz .*$/double Bz = ${BFIELD}e\-1;/" cocktailInput.cxx

### set TOF radius
sed -i -e "s/set barrel_Radius .*$/set barrel_Radius ${TOFRAD}e\-2/" propagate.tcl
sed -i -e "s/double tof_radius = .*$/double tof_radius = ${TOFRAD}\;/" cocktailInput.cxx
### set TOF length
sed -i -e "s/set barrel_HalfLength .*$/set barrel_HalfLength ${TOFLEN}e\-2/" propagate.tcl
sed -i -e "s/double tof_length = .*$/double tof_length = ${TOFLEN}\;/" cocktailInput.cxx
### set TOF acceptance
...
sed -i -e "s/double tof_sigmat = .*$/double tof_sigmat = ${SIGMAT}\;/" cocktailInput.cxx
sed -i -e "s/double tof_sigma0 = .*$/double tof_sigma0 = ${SIGMA0}\;/" cocktailInput.cxx
...
wait
hadd -f cocktailInput.rmin${RADIUS}.${BFIELD}kG.${SYSTEM}.root cocktailInput.*.root &&
mv cocktailInput.rmin${RADIUS}.${BFIELD}kG.${SYSTEM}.root ./output/
rm -rf cocktailInput.*.root &&
